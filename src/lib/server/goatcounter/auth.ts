...
/**
 * Attempt to authenticate with GoatCounter using email + password.
 *
 * GoatCounter's login flow POSTs to /user/requestlogin with form data.
 * On success it redirects; on failure it returns an error page.
 */
export async function authenticateWithGoatCounter(
	...
	try {
		...
		if (body.includes('TOTP') || body.includes('totp') || body.includes('two-factor')) {
			return {
				success: false,
				reason: 'mfa_required'
			};
